# Leetcode Challenge

Daily [leetcode](https://leetcode.com/problemset/all/) Challenge.

Languages used: Java and Python

---

## Navigation

### [Array](./Array)

- [Add to Array-Form of Integer](Array/AddToArrayFormInteger)
- [Container With Most Water](Array/ContainerWithMostWater)
- [Diagonal Traverse II](Array/DiagonalTraverse2)
- [Divide Arrays In K Consecutive Numbers](Array/DivideArrayInKConsecNums)
- [Elimination Game](Array/Elimination)
- [Fair Candy Swap](Array/FairCandySwap)
- [Find Common Characters](Array/FindCommonCharacters)
- [Find the Duplicate Number](Array/FindDuplicateNumber)
- [Find Lucky Integer in Array](Array/FindLuckyIntegerInArray)
- [Find the Mediam From Data Stream](Array/FindMediamFromDataStream)
- [Find Minimum in Rotated Sorted Array](Array/FindMinInRotatedSortedArray)
- [First Missing Positive](Array/FirstMissingPositive)
- [Increasing Triplet Subsequence](Array/IncreasingTripletSubsequence)
- [Insert Interval](Array/InsertInterval)
- [Intersection of Two Arrays](Array/IntersectionOfTwoArrays)
- [K Closest Points to Origin](Array/KClosestPointsToOrigin)
- [Longest Consecutive Sequence](Array/LongestConsecutiveSequence)
- [Majority Element](Array/MajorityElement)
- [Maximize Distance to Closest Person](Array/MaximizeDistanceToClosestPerson)
- [Max Product of Three Numbers](Array/MaxProductofThreeNumbers)
- [Minimum Value to Get Positive Step by Step Sum](Array/MinValStepSum)
- [N-Repeated Element in Size 2N Array](Array/N_RepeatedElementInArray)
- [Next Greater Element](Array/NextGreaterElement1)
- [Partition Array Into Disjoint Intervals](Array/PartitionArrayIntoDisjointIntervals)
- [Pascal's Triangle](Array/PascalsTriangle)
- [Prison Cells After N Days](Array/PrisonCellsAfterNDays)
- [Queens That can Attack King](Array/QueensThatCanAttackKing)
- [Rank Transform Array](Array/RankTransformArray)
- [Reshape the Matrix](Array/ReshapeMatrix)
- [Rotate the Array](Array/RotateArray)
- [Search in Rotated Sorted Array](Array/SearchInRotatedSortedArray)
- [Shortest Unsorted Continuous Subarray](Array/ShortestUnsortedContinuousSubarray)
- [Shuffle Array](Array/ShuffleArray)
- [Sort Array by Parity](Array/SortArrayByParity)
- [Sort Colors](Array/SortColors)
- [Subarray Sum Equals K](Array/SubarraySumEqualsK)
- [Sum Of Even Numbers After Queries](Array/SumOfEvenNumbersAfterQueries)
- [Third Maximum Number](Array/ThirdMaximumNumber)

### [Bit Manipulation](./BitManipulation)

- [Binary Prefix Divisible by 5](Array/BinaryPrefixDivBy5)
- [Single Number II](BitManipulation/SingleNumber2)
- [Total Hamming Distance](BitManipulation/TotalHammingDistance)

### [Dynamic Programming](./DynamicProgramming)

- [Climbing Stairs](DynamicProgramming/ClimbingStairs)
- [Count Square With Ones](DynamicProgramming/CountSquareWithOnes)
- [House Robber](DynamicProgramming/HouseRobber)
- [Longest Increasing Subsequence](DynamicProgramming/LongestIncreasingSubsequence)
- [Longest Palindromic Substring](DynamicProgramming/LongestPalindromicSubstring)
- [Maximum Length Pair Chain](DynamicProgramming/MaximumLengthPairChain)
- [Max with Min Altitudes](DynamicProgramming/MaxWithMinAltitudes)
- [Number Of Islands](DynamicProgramming/NumberOfIslands)
- [Palindromic Substrings](DynamicProgramming/PalindromicSubstrings)
- [Predict Winner](DynamicProgramming/PredictWinner)
- [Reducing Dishes](DynamicProgramming/ReducingDishes)
- [Regex Matching](DynamicProgramming/RegexMatching)
- [Unbounded Knapsack](DynamicProgramming/UnboundedKnapsack)
- [Unique Paths](DynamicProgramming/UniquePaths)
- [0-1 Knapsack](DynamicProgramming/ZeroOneKnapsack)

### [Graph](./Graph)

- [All Paths From Src To Target](Graph/AllPathsFromSrcToTarget)
- [Course Schedule II](Graph/CourseSchedule2)
- [Escape Large Maze](Graph/EscapeLargeMaze)
- [Minimum Cost to Repair Edges](Graph/MinCostToRepairEdges)
- [Number to Make Network Connected](Graph/NumToMakeNetworkConnected)
- [Remove Duplicates from Sorted List](Array/RemoveDuplicatesFromSortedList)
- [Reconstruct Itinerary](Graph/ReconstructItinerary)
- [Surrounded Regions](Graph/SurroundedRegions)

### [Greedy](./Greedy)

- [Assign Cookies](Greedy/AssignCookies)
- [Car Pooling](Greedy/CarPooling)
- [Fractional Knapsack](Greedy/FractionalKnapsack)
- [Maximize Sum after K Negations](Greedy/MaximizeSumAfterKNegations)
- [Maximum Score Words](Greedy/MaximumScoreWords)
- [Score After Flipping Matrix](Greedy/ScoreAfterFlippingMatrix)

### [LinkedList](./LinkedList)

- [Design Linked List](LinkedList/DesignLinkedList)
- [Linked List Cycle II](LinkedList/LinkedListCycle2)
- [Merge Two Sorted Lists](Array/MergeTwoSortedLists)
- [Reverse Linked List II](LinkedList/ReverseLinkedList2)
- [Swap Nodes in Pairs](LinkedList/SwapNodesInPairs)

### [Math](./Math)

- [Find the Minimum Number of Fibonacci Numbers Whose Sum Is K](Math/FindMinNumFibNumSumK)
- [Happy Number](Math/HappyNumber)
- [Maximum Swap](Math/MaximumSwap)
- [Perfect Squares](Math/PerfectSquares)
- [Square Root X](Math/SqrtX)

### [Recursion](./Recursion)

- [Combinations](Recursion/Combinations)
- [Permutations](Recursion/Permutations)
- [Permutations II](Recursion/Permutations2)
- [Permutation Sequence](Recursion/PermutationSequence)

### [Stack](./Stack)

- [Asteroid Collision](Stack/AsteroidCollision)
- [Backspace String Compare](Stack/BackspaceStringCompare)
- [Minimum Add to Make Parentheses Valid](Stack/MinimumAddtoMakeParenthesesValid)
- [Reverse Substrings Between Parentheses](Stack/ReverseSubstringsBetweenParentheses)

### [String](./String)

- [Isomorphic Strings](String/IsomorphicStrings)
- [Longest Common Prefix](String/LongestCommonPrefix)
- [Minimum Steps to Make Two Strings Anagram](String/MinStepsToMakeTwoStringsAnagram)
- [Rearrange Words in Sentence](String/RearrangeWordsInSentence)
- [String Matching in Array](String/StringMatchingInArray)
- [Substring with Exactly K Distinct Characters](String/SubstrWithExactKDistinctChars)

### [Tree](./Tree)

- [All Elements in Two BSTs](Tree/AllElementsInTwoBSTs)
- [Binary Postorder](Tree/BinaryPostorder)
- [Check Valid Tree Given Edges](Tree/CheckValidTreeGivenEdges)
- [Construct Binary Tree](Tree/ConstructBinaryTree)
- [Distribute Coins in Binary Tree](Tree/DistributeCoinsInBinaryTree)
- [Invert Binary Tree](Tree/InvertBinaryTree)
- [Merge Two Binary Trees](Tree/MergeTwoBinaryTrees)
- [N-ary Tree Postorder Traversal](Tree/NaryTreePostorderTraversal)
- [Serialize and Deserialize Binary Tree](Tree/SerializeAndDeserializeBinaryTree)

### [Others](./Others)

- [Apply Discount Every N Orders](Others/ApplyDiscountEveryNOrders)
- [Design Browser History](Others/DesignBrowserHistory)
